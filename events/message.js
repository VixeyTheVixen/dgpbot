  ...
  if (message.channel.id === process.env.APRESENTACAO) {
    ...
    if (!message.member.roles.exists('name', role.name)) {
      ...
      const embed = {
        ...
        description: '**Hey**, caso você tenha errado a digitação de algo em sua mensagem de apresentação, basta digitar o comando `p!reset` no *chat de <#424328750323204127> do servidor* para resetar a sua apresentação!'
      }
      message.author.send({
        embed: embed
      })
